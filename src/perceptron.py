...
def perceptron_train(X, Y):
    ...
    x_pos_vals = [val for i, val in enumerate(x_values) if Y[i] == 1]
    y_pos_vals = [val for i, val in enumerate(y_values) if Y[i] == 1]
    x_neg_vals = [val for i, val in enumerate(x_values) if Y[i] == -1]
    y_neg_vals = [val for i, val in enumerate(y_values) if Y[i] == -1]
